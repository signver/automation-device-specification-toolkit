...
import { ReadWriteRequestStage, RequestStage } from '../common'
import requestWorkflow from './request'

type AMSReadRequestPacket = AMSPacket<ADSReadWriteRequest>

let message: AMSPacket | AMSReadRequestPacket
let workflow: RequestStage
let flow: ReadWriteRequestStage
function initialize() {
  message = {
    header: {
      command: 0,
...
describe("readWrite", () => {
  ...
  describe("workflow", () => {
    ...
    it("should set read length", () => {
      const input = 0xfe
      expect(flow.length(input).length()).toStrictEqual(input)
